...
export default function SignUp(){
  ...
  useEffect(() => {
    async function fetchData() {
      const apiClients = await backendService.clients.getAllClients();
  ...
  async function handleAddPost(event) {
    ...
    if (!fullName || !idCard || !address || !mobilePhone) {
    ...
    backendService.clients.addClient(newPost).then((newPost) => {
